...
def icosphere_directions(n_subdiv: int = 2) -> "np.ndarray":
    ...
    FreeZeV2 Sec. IV-A renders "162 templates per object using the camera
    viewpoints proposed by CNOS"; CNOS's template views are icosphere vertices,
    and 162 is the second subdivision (10*4^n + 2). The historical placement
    here was a golden-ratio spiral - same count, different directions - which
    is ledger row #21. Deterministic: no RNG anywhere.
    """
    t = (1 + 5 ** 0.5) / 2
...
class QueryFeatureExtractor:
    ...
    def extract_query_features(self, mesh_path, pts_query, n_views=None):
        ...
        # View placement (ledger #21). "spiral" is the historical golden-ratio
        # spiral; "ico162" is the icosphere family the paper's "camera
        # viewpoints proposed by CNOS" belongs to. Radius stays ours (the
        # paper names no radius). A cache-key knob in bop_eval's enc_cfg.
